...
# zhaogj CA Generation
openssl req -nodes \
    -newkey rsa:2048 \
    -keyout $home_ca/private/zhaogj-ca.key \
    -out $home_ca/csrs/zhaogj-ca.csr \
    -sha256 \
    -batch \
    -subj "/CN=ZHAOGJ CA"

openssl x509 -req \
    -in $home_ca/csrs/zhaogj-ca.csr \
    -out $home_ca/crts/zhaogj-ca.crt \
    -CA $home_ca/crts/sub-ca.crt \
    -CAkey $home_ca/private/sub-ca.key \
    -sha256 \
    -days 4383 \
    -CAcreateserial \
    -extensions zhaogj_ca_ext \
    -extfile build-ca.conf
